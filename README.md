...
# AE 100 NI USB6009 Force Logger v1.0.0
...
2. **Disable Fast Startup in Control Panel -> Hardware Settings -> Power Options -> choose what the power buttons do -> Uncheck "Turn on Fast Startup"**. Having this setting on seems to interfere with NI driver installation, and will also cause BSODs with NI drivers installed (from experience with something similar). With Fast Startup disabled, startup should take ~30 s longer with SSD, longer if you have a hard drive. (but now there should be greater system stability after a restart)
...
9. You can pan/zoom by either dragging/scrolling the graph around with your mouse, or using the X/Y Min/Max range input boxes. Once you are done zooming, hit the home button on the bottom of the graph in the Matplotlib toolbar, or set the X/Y Min/Max ranges to a reasonable number to get ready for the next test. Note that if you use the X Min/Max range inputs, it will not automatically extend to fit new data like it does on startup, which is why I recommend using the Matplotlib toolbar.
10. When done/you want to reload the program, simply close the window.
...
Software works by interfacing with the separate NI-DAQmx drivers running in the background. Quick diagram:



**Force from motor --> load cell --> USB6009 DAQ hardware --> NI-DAQmx drivers on computer --> NI\_USB6009\_ForceLogger.exe**



The current setup uses the USB6009 DAQ from National Instruments that records load cell readings in voltage from close to 0.05 V to more than 3 V. 
...
* Entirety of packages: matplotlib, nidaqmx, tkinter, along with all those dependencies (including numpy)
  ...
  * All the packages included in the exe can be found in the environment.yml file extracted from Conda (that you can reimport into Miniconda/Anaconda)
...
* My understanding is all NI DAQs are pretty much interchangeable through the DAQmx API (except some of the more advanced ones with more features)
...
* Basically initialize a new Conda environment with minimum required packages, then install the rest from pip instead. Makes the compiled executable file way smaller, from 190 MB to 45 MB.
